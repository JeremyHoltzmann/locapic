...
function MapScreen(props) {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("POIList", function (err, data) {
      props.initPOI(JSON.parse(data));
    });
    askPermissions();
  }, []);
  ...
  function addPOIFunction() {
    ...
    AsyncStorage.setItem("POIList", JSON.stringify(tmpPOIList));
    props.setPOI(newPOI);
    setTitle("");
    setDescription("");
    setPOILocation({});
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    initPOI: function (POIList) {
      dispatch({ type: "initPOI", POIList });
    },
  };
}
